...
<div class=" justify-content-end gap-2 p-4">
    <div class="col-md-6">
        <form action="{{ route('smartphone.import') }}" method="POST" enctype="multipart/form-data" class="d-flex flex-column gap-2 shadow-sm p-3 bg-white rounded">
            @csrf
    ...
    <div class=" gap-2">
        <a href="{{ route('scanner') }}" class="btn btn-outline-secondary btn-sm align-items-center gap-2">
        ...
        <a href="/smartphones/csv-all" target="_blank" class="btn btn-primary btn-sm rounded-2" style="font-size: 14px;">CSV</a>
        <a href="/smartphones/pdf" target="_blank" class="btn btn-primary btn-sm rounded-2" style="font-size: 14px;">Export</a>
    </div>
</div>
        ...
        <div class="col-md-3 mb-4">
            <div class="card">
                <div class="card-body text-center">
                    <img src="{{ asset($smartphone->image) }}" alt="{{ $smartphone->name }}" class="img-fluid mb-2" style="max-height: 200px;">
                    <h5 class="card-title">{{ $smartphone->brand_name }}</h5>
                    <p class="card-text">{{ $smartphone->description }}</p>
                    <p class="card-text">{{ $smartphone->price }}</p>
                    {!! QrCode::size(100)->generate(Request::url() . '/smartphones/' . $smartphone->id) !!}
                </div>
            </div>
        </div>
        @endforeach
